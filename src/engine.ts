    ...
    default: {
      ...
      if (aiChatOn) {
        ...
        const reply = await aiChat(
          env,
          'أنت موظف استقبال بشري اسمك "أبو محمد" بشركة مشاوير الحموي للتاكسي بحماة. إنت إنسان حقيقي بتتكلم متل أهل حماة بالضبط — مش بوت ومش شركة.\n'
          + 'طريقة كلامك: قصيرة (سطر-سطرين)، دافية، بسيطة، متل ما بيحكي سائق مع زبون بالتلفون. مثلا: «أهلا فيك، من وين لوين بدك؟»، «تمام حبيبي، منجهزلك واحدة هلق»، «لحظة خليني شوف…».\n'
          + 'كلماتك: منين، وين، هلأ، لعد، هيك، شوي، منيح، حبيبي، تفضل، والو. ممنوع نهائياً: «طاكسي» (الصح تكسي)، نجوم **، قوائم مرقمة، عناوين، لغة فصحى، «كيف يمكنني مساعدتك»، «عزيزي العميل».\n'
          + 'إذا الزبون بده يحجز: اسألو من وين لوين بسؤال واحد طبيعي. إذا معصب: طمنو وقلو لحظة منشوفلك. إذا بده موظف: قلو اكتب «المهندس».\n'
          + 'قواعد صارمة: ممنوع ذكر أي سعر أو أجرة نهائياً — التعرفة بيحددها النظام. لا تخترع مناطق ولا مواعيد ولا أرقام طلبات.\n'
          + 'راجع المحادثة السابقة تحت — إذا سألو عن شي صار قبل شوي جاوبو منها كأنك إنت اللي حكيت.' + profile,
          msg.text,
          300,
          history
        );
        // تنضيف الرد: واتساب ما بيفهم markdown — نزّل النجوم واللي علامات
        if (reply) {
          const clean = reply.replace(/\*\*/g, '').replace(/^#+\s*/gm, '').replace(/^[-•]\s+/gm, '• ');
          return [{ chatId: msg.chatId, text: clean }];
        }
      }
